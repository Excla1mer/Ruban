...
int main(){
	...
	cout<<"---------------------------------------\n";
	for(i=0;i<n;i++)		//Perestanovka strok po ybivaniy diag. el-ta
	{	
		for(j=0;j<n;j++)
		{
			if(i==j)
	...
	for(i=0;i<n;i++){			//Proverka diag. preobladaniya
		for(j=0;j<n;j++){
			if(i==j)
				maxs=a[i][j];
			else
...
if(b==true)
{
	for(i=0;i<n;i++){  //Sozdanie matrichi C
		for(j=0;j<n;j++){
			if(i!=j){
				a[i][j]=(a[i][j]/a[i][i])*-1;
			}
